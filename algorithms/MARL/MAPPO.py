...
config_dir = os.path.abspath(os.path.dirname(__file__)) + '/configs/configs_ppo.ini'
...
class MAPPO:
    ...
    def interact(self):
        ...
        for i in range(self.roll_out_n_steps):
            ...
            next_state, global_reward, done, info = self.env.step([index_to_one_hot(a, self.action_dim) for a in action])
    ...
    def evaluation(self, env, eval_episodes=1, is_train=True):
        ...
        for i in range(eval_episodes):
            ...
            while not all(done):
                ...
                state, reward, done, info = env.step([index_to_one_hot(a, self.action_dim) for a in action])
        ...
        env.close()
        return rewards, steps, avg_speeds
